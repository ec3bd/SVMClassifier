...
def main():
    ...
    modelWithParam = clf.train_and_select_model(xMat, yMat)   #what I use to do cv and pick the best parameters out of those in param_set, returns a model with those params encoded
                                                                #comment out if it's too slow, use the method below to see my prediction generation

    modelWithParam = svm.SVC(kernel = 'rbf', C=50, gamma = 25)  #for generating prediction files a bit quicker towards the end
